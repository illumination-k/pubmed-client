//! Common test utilities for PMC XML test cases
#![allow(dead_code)]
...
pub fn get_pmc_xml_test_cases() -> Vec<PmcXmlTestCase> {
    let xml_dir_workspace = Path::new("pubmed-formatter/tests/integration/test_data/pmc_xml");
...
pub fn get_pmc_xml_test_case(filename: &str) -> Option<PmcXmlTestCase> {
    let xml_path_workspace =
        Path::new("pubmed-formatter/tests/integration/test_data/pmc_xml").join(filename);
