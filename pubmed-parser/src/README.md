...
3. String `find()` and slicing should not be used for XML structure extraction.
...
Legacy XML structure helpers such as `extract_text_between`, `find_all_tags`, and `extract_element_content` have been removed.
String-based XML extraction can silently miss malformed XML, match tag-like text inside comments or character data, and depend on exact attribute order or whitespace.
