...
/// Parser for extracting article metadata from PMC XML content
            ...
            if let Some(funding_end) = content[funding_start..].find("</funding-group>") {
                ...
                while let Some(award_start) = funding_section[pos..].find("<award-group") {
                    ...
                    if let Some(award_end) = funding_section[award_start..].find("</award-group>") {
                        ...
                        funding_info.award_id = xml_utils::extract_text_between(
                            award_section,
                            "<award-id>",
                            "</award-id>",
                        );
    ...
    pub fn extract_supplementary_materials(content: &str) -> Vec<SupplementaryMaterial> {
        ...
        while let Some(supp_start) = content[pos..].find("<supplementary-material") {
            ...
            if let Some(supp_end) = content[supp_start..].find("</supplementary-material>") {
                ...
                let id =
                    xml_utils::extract_attribute_value(supp_content, "id").unwrap_or_else(|| {
                ...
                let caption =
                    xml_utils::extract_text_between(supp_content, "<caption>", "</caption>")
                    ...
                    .or_else(|| {
                        ...
                        if let Some(media_start) = supp_content.find("<media") {
                            if let Some(media_end) = supp_content[media_start..].find(">") {
                                let media_tag =
                                    &supp_content[media_start..media_start + media_end + 1];
                                xml_utils::extract_attribute_value(media_tag, "xlink:href").or_else(
                                    || xml_utils::extract_attribute_value(media_tag, "href"),
                                )
                            } else {
                                None
                            }
...
mod tests {
    ...
    fn test_extract_title() {
        ...
        let title = MetadataParser::extract_title(content);
    ...
    fn test_extract_doi() {
        ...
        let doi = MetadataParser::extract_doi(content);
    ...
    fn test_extract_pmid() {
        ...
        let pmid = MetadataParser::extract_pmid(content);
    ...
    fn test_extract_keywords() {
        ...
        let keywords = MetadataParser::extract_keywords(content);
    ...
    fn test_extract_pub_date() {
        ...
        assert_eq!(MetadataParser::extract_pub_date(content_full), "2023-12-25");

        let content_year_month = r#"<year>2023</year><month>12</month>"#;
        assert_eq!(
            MetadataParser::extract_pub_date(content_year_month),
            "2023-12"
        );

        let content_year_only = r#"<year>2023</year>"#;
        assert_eq!(MetadataParser::extract_pub_date(content_year_only), "2023");

        let content_no_date = r#"<title>No date here</title>"#;
        assert_eq!(
            MetadataParser::extract_pub_date(content_no_date),
            "Unknown Date"
        );
    }
    ...
    fn test_extract_article_type() {
        ...
        let article_type = MetadataParser::extract_article_type(content);
    ...
    fn test_extract_language() {
        ...
        let language = MetadataParser::extract_language(content);
    ...
    fn test_extract_article_ids() {
        ...
        let ids = MetadataParser::extract_article_ids(content);
        assert_eq!(ids.len(), 3);
    ...
    fn test_extract_acknowledgments() {
        ...
        let ack = MetadataParser::extract_acknowledgments(content);
        assert_eq!(
            ack,
