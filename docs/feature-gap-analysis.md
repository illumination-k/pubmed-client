...
調査した全ライブラリ (Bio.Entrez, PyMed, metapub, rentrez, pubmed.mineR, easyPubMed, entrezpy, EDirect, pubmedR, reutils, node-ncbi, ncbi-eutils, pubmed crate) のいずれも提供していない、当プロジェクト独自の機能:

| 機能                          | 説明                                                                                 |
| ----------------------------- | ------------------------------------------------------------------------------------ |
| PMC フルテキスト構造的パース  | PMC XML をセクション (Introduction, Methods, Results, Discussion) に構造化して解析   |
| PMC Markdown 変換             | フルテキストを設定可能な Markdown に変換 (TOC, YAML frontmatter, 参考文献スタイル等) |
| PMC 図表抽出 (tar.gz)         | PMC OA API から tar.gz をダウンロードし、図表とキャプションを自動マッチング          |
| PubMed + PMC 統合クライアント | メタデータ検索とフルテキスト取得を単一のワークフローで実行                           |
| MCP サーバー                  | AI アシスタント (Claude Desktop等) からの直接利用                                    |
| マルチ言語バインディング      | 単一の Rust コアから Python (PyO3) + Node.js (NAPI) + WASM の 4 プラットフォーム     |
| 型安全クエリビルダー          | フィールド別フィルタ、日付範囲、ブーリアン論理をビルダーパターンで構築               |
| TTL 付きインメモリキャッシュ  | moka ベースの設定可能なキャッシュ (TTL, 容量制限)                                    |
| PMC OA アーカイブ抽出         | PMC Open Access の tar.gz からファイルを抽出・処理                                   |
| MeSH 類似度計算               | Jaccard 類似度による論文間 MeSH 比較                                                 |
| 国際共同研究検出              | 著者所属国の解析による自動判定                                                       |
| 非同期ストリーミング          | 大規模結果セットの async Stream 取得                                                 |

**特記**: 調査した全13ライブラリにおいて、PMC フルテキストの構造的パース・Markdown 変換・図表抽出を提供するものは**皆無**であった。これは pubmed-client の最大の差別化要因である。
